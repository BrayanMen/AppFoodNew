...
function Footer() {
  ...
  return (
    <div className=' bg-greenP py-4 border=t-2 border-lime-900 '>
      <div className='container mx-auto px-2'>
        <div className='grid grid-cols-2 text-white md:grid-cols-7 xl:grid-cols-12 gap-5 sm:gap-9 lg:gap-11 py-10 justify-between'>
              ...
              <ul className='text-sm flex flex-col space-y-3'>
                {link.links.map((text, index)=>(
                  <li key={index} className='flex items-baseline'>
                    <Link to={text.link} className='text-gray-300 inline-block w-full hover:text-secondary'>
                      {text.name}
                      </Link>
                  </li>
                ))}
